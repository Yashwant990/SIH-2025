...
app.config.update({
    ...
    "SESSION_COOKIE_SAMESITE": "Lax" # Extra security
})
Session(app)
...
def upload_resume():
    ...
    try:
        ...
        from tasks import parse_resume_task
        task = parse_resume_task.delay(text, session["user_id"], aspiration)

        # Save task ID so we can check later
        session['resume_task_id'] = task.id

        flash("✅ Resume uploaded! Processing in the background...")
        return redirect(url_for("dashboard"))
...
# app.py (Focus on /resume_status route)

@app.route("/resume_status")
def resume_status():
    task_id = session.get("resume_task_id")
    if not task_id:
        return jsonify({"status": "NO_TASK"})

    task = parse_resume_task.AsyncResult(task_id)

    if task.state == "SUCCESS":
        result = task.result

        # === START OF CRITICAL FIX ===
        # 1. Save the task result into the Flask session for the dashboard
        session['analysis_results'] = {
            "skills": result.get("skills", []),
            "careers": result.get("careers", []),
            "aspiration": session.get('aspiration', '') # Ensure aspiration is saved too if needed
        }
        # 2. Clear the task ID now that the result is processed
        session.pop('resume_task_id', None)
        # === END OF CRITICAL FIX ===
        ...
        # Also ensure failure clears the task ID to avoid infinite checking
        session.pop('resume_task_id', None)
...
def upgrade_user_profile_table():
    ...
    try:
        ...
        pass  # Column already exists
    # ... (rest of the ALTER TABLE statements) ...
    conn.commit()
    conn.close()    
...
if __name__ == "__main__":
    init_db() 
    # === CRITICAL FIX: Run the schema upgrade ===
    upgrade_user_profile_table()
    # ===========================================
